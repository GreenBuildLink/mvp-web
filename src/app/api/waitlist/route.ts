...
const companyPayloadSchema = z.object({
    ...
    companyType: z.string().trim().min(1).max(160),
    yearsOfOperation: z.string().trim().min(1).max(160),
    mainSector: z.string().trim().min(1).max(160),
        ...
        try {
            if (type === "project") {
                ...
                await insertCompanySubmission({
                    ...
                    company_type: text(rest.companyType),
                    years_of_operation: text(rest.yearsOfOperation),
                    main_sector: text(rest.mainSector),
